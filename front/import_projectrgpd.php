...
die();
  ...
  function delete_modelrgpd_project($project_id)
   {
	global $DB;	   
		// `glpi_tickettasks` (id, uuid, tickets_id)
...
// STEP 2 : on supprime le projet existant si il existe
	$result = $DB->query('SELECT `id` FROM `glpi_projects` WHERE `name` = "Référentiel RGPD"');
	$project_id = 0;
	if ($result && $DB->numrows($result) > 0) {
		$data = $DB->fetchAssoc($result); $project_id = $data['id']; 
		delete_modelrgpd_project($project_id);
		//header("Refresh:0; url=config.form.php");
		Session::addMessageAfterRedirect("Ancien projet effacé");
	}
...
// 'glpi_projecttasks_tickets' : no id, 
  ...
  // STEP 4 : update relation id  
	...
	// `glpi_projecttasks` (id, uuid, name, entities_id, projects_id, projecttasks_id) // une tâche ne peut appartenir qu'à un seul projet
			...
			$DB->queryOrDie($query, $DB->error());
			$query = "UPDATE `glpi_projecttasks` AS T1 INNER JOIN `glpi_projecttasks` AS T2  
							ON T1.projecttasks_oid = T2.id 
							SET T1.`projecttasks_id` = T2.`oid`";
			//$DB->queryOrDie($query, $DB->error());
					// `glpi_tickets` (id, name, entities_id)
   ...
   foreach ($tables as $table) {
	...
	$DB->queryOrDie($query, $DB->error());
    Session::addMessageAfterRedirect("Projet Conformité RGPD importé");
	header("Refresh:0; url=config.form.php");  