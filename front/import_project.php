...
// importation des .dat du projet modèle vers l'entité actuelle
// STEP 1 : recup de l'id de l'entité actuelle
$DB->request("SET @currentuser_id := ".Session::getLoginUserID());
$DB->request("SET @currententity_id := ".Session::getActiveEntity());

$target_entities_id = $_POST["entities_id"]??Session::getActiveEntity();
// var_dump (Session::getActiveEntity()) ; die ;

// si entité atuelle = "model-rgpd" alors on quitte
	$result = $DB->query('SELECT * FROM `glpi_entities` WHERE `name` = "model-rgpd"');
	$modelrgpd_id = 0;
	if ($result && $DB->numrows($result) > 0) {
		$data = $DB->fetchAssoc($result); $modelrgpd_id = $data['id'];
	}
	if ($target_entities_id == $modelrgpd_id) {
	// exit;
	}
...
// 'glpi_projecttasks_tickets' : no id,
  ...
  // STEP 4 : update relation id
	// `glpi_projects` (id, name, enetities_id)
	$query = "UPDATE `glpi_projects` SET entities_id = $target_entities_id where `oid` <> 0";
    $DB->queryOrDie($query, $DB->error());
	...
	// `glpi_projecttasks` (id, uuid, name, entities_id, projects_id, projecttasks_id) // une tâche ne peut appartenir qu'à un seul projet
			$query = "UPDATE `glpi_projecttasks` AS T1 INNER JOIN `glpi_projects` AS T2  
						...
						SET T1.`entities_id` = $target_entities_id, T1.`projects_id` = T2.`id`";
			$DB->queryOrDie($query, $DB->error());
			...
			$DB->queryOrDie($query, $DB->error());
			$query = "SELECT COUNT(*) FROM `glpi_projecttasks` WHERE `entities_id` = '$target_entities_id'";
			$row = $DB->query($query)->fetch_assoc();
			$message .= strval($row["COUNT(*)"]) . " sous-projets importées <br>";
					// `glpi_tickets` (id, name, entities_id)
					$query = "UPDATE `glpi_tickets`AS T1
								SET T1.`entities_id` = $target_entities_id
								WHERE T1.`oid` <> 0";
					$DB->queryOrDie($query, $DB->error());
