...
if (isset($_POST["rename_tab"])) {
    $tab = new PluginDlteamsProcedure_Section();
...
if (isset($_POST["moveup_tab"])) {
    $allchapter = $DB->request(PluginDlteamsProcedure_Section::getTable(), ['ORDER' => ['timeline_position ASC'], 'procedures_id' => $_POST["procedures_id"]]);
    $tab = new PluginDlteamsProcedure_Section();
    $tab->getFromDB($_POST["tabnum"]);
    $previous_tab = null;
...
//                et on donne le numero de notre onglet a l'onglet précédent
                $pt = new PluginDlteamsProcedure_Section();
                $pt->update([
...
if (isset($_POST["movedown_tab"])) {
    $allchapter = $DB->request(PluginDlteamsProcedure_Section::getTable(), ['ORDER' => ['timeline_position DESC'], 'procedures_id' => $_POST["procedures_id"]]);
    $tab = new PluginDlteamsProcedure_Section();
    $tab->getFromDB($_POST["tabnum"]);
    $previous_tab = null;
    $current_tabnum = $tab->fields["timeline_position"];
...
if (isset($_GET["add_tab"])) {
    $position = 0;
    $chapter = new PluginDlteamsProcedure_Section();
    $condition = [
        'FROM' => 'glpi_plugin_dlteams_procedures_sections',
        'ORDER' => 'id DESC',
        'LIMIT' => 1,
        'procedures_id' => $_GET["procedures_id"]
    ];
...
if (isset($_POST["save_section"])) {
    ...
    foreach ($_POST["name"] as $key => $name) {
        if ($key === 0) {
            $section = new PluginDlteamsProcedure_Section();

            $section->update([
                "name" => $_POST["name"][$key],
                "comment" => $_POST["comment"][$key],
                "content" => $_POST["content"][$key],
                'id' => $_POST["procedures_sections_id"]
            ]);

            Session::addMessageAfterRedirect("Section mis à jour avec succès");
        } else {
            $procedurecontent = new PluginDlteamsProcedure_Content();

            if (isset($_POST["id_field"]) && $_POST["id_field"][$key]) {
                $procedurecontent->update([
                    "name" => $_POST["name"][$key],
                    "comment" => $_POST["comment"][$key],
                    "content" => $_POST["content"][$key],
                    "timeline_position" => $key,
                    "procedure_sections_id" => $_POST["procedures_sections_id"],
