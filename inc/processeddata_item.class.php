...
class PluginDlteamsProcessedData_Item extends CommonDBTM
{
    public static $itemtype_1 = "PluginDlteamsProcessedData";
    public static $items_id_1 = 'processeddatas_id';
    ...
    {
        return __("Processed Data", 'dlteams');
    }
    ...
    public function getTabNameForItem(CommonGLPI $item, $withtemplate = 0)
    {
        $nbdoc = $nbitem = 0;
        switch ($item->getType()) {
            case 'PluginDlteamsProcessedData':
                $ong = [];
...
//                self::showForitem($item, $withtemplate);
    ...
    public static function showItems(PluginDlteamsProcessedData $object_item)
    {
        global $DB;
...
//        Enlève le choix de L'objet LegalBasi dans la dropdown qui affiche la liste des objets
        $key = array_search("PluginDlteamsProcessedData", $types);
        unset($types[$key]);
        $rand = mt_rand();
        ...
        if ($canedit) {
            ...
            echo __("Comment");
            echo "<br/><br/>";
            echo "<textarea type='text' style='width:100%' maxlength=1000 rows='3' name='comment' class='comment_processedsdata_item'></textarea>";
            echo "</td>";
            echo "</table>";
            Html::closeForm();
        }
...
//            ],
    ...
    public static function showItemsForItemType(CommonDBTM $object_item)
    {
        ...
        if ($canedit) {
            echo "<form name='processeddataitem_item_form$rand' id='processeddataitem_item_form$rand' method='post'
            action='" . Toolbox::getItemTypeFormURL(__CLASS__) . "'>";
...
//            echo "<input type='hidden' name='records_id' value='" . $item->getID() . "' />";
            ...
            $title = __("Add Processed Data", 'dlteams');

            $entitled = "Add Processed Data to record";
            echo "<table class='tab_cadre_fixe'>";
            ...
            echo "<td width='20%' class='left'><input for='processeddataitem_item_form$rand' type='submit' name='add' value=\"" . _sx('button', 'Add') . "\" class='submit'>";
            echo "</td></tr>";
            echo "</table>";
                    ...
                    $(document).ready(function () {
                        ...
                        if ($(this).val() != '0') {   
                            ...
                            $.ajax({
                                ...
                                data: {
                                    id: $(this).val(),
                                    object: 'PluginDlteamsProcessedData',
                                    field: 'comment'
                                },
...
//                Html::openMassiveActionsForm();
            ...
            foreach ($iterator as $data) {
                ...
                if ($data['name']) {
                    ...
                    $lbt = new PluginDlteamsProcessedDatasType();
    ...
    public  static function getRequest(CommonDBTM $item){
        $params = [
            'SELECT' => [
                'glpi_plugin_dlteams_processeddatas_items.id AS linkid',
                'glpi_plugin_dlteams_processeddatas_items.comment AS comment',
                'glpi_plugin_dlteams_processeddatas.' . 'name as name',
                'glpi_plugin_dlteams_processeddatas.' . 'id as id',
                'glpi_plugin_dlteams_processeddatas.content',
                'glpi_plugin_dlteams_processeddatas.plugin_dlteams_processeddatastypes_id as processeddatastypes_id',
            ],
            'FROM' => 'glpi_plugin_dlteams_processeddatas_items',
            'JOIN' => [
            ...
            'WHERE' => [
                'glpi_plugin_dlteams_processeddatas_items.items_id' => $item->fields['id'],
                'glpi_plugin_dlteams_processeddatas_items.itemtype' => $item->getType(),
...
//            echo "<input type='hidden' name='records_id' value='" . $item->getID() . "' />";
    ...
    {
        $legalbasi_item = new PluginDlteamsProcessedData_Item();
...
//            mis a jour de record
            ...
            $record_item->getFromDBByCrit([
                ...
                "items_id" => $legalbasi_item_oldfields["processeddatas_id"],
            ...
            if($record_item){
                $DB->update(
                    ...
                    [
                        "items_id" => $input["plugin_dlteams_processeddatas_id"]
                    ],
                    [
...
//        purge relations
        ...
        $criteria = [
            "itemtype" => PluginDlteamsProcessedData::class,
            "items_id" => $this->fields["processeddatas_id"],
