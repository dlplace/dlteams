...
class PluginDlteamsTiersCategory_onglet extends CommonDBTM {
 ...
 //var_dump($id);
   ...
   static function getRequest($record) {
     return [
         ...
         'WHERE' => [
            'glpi_plugin_dlteams_records_externals.plugin_dlteams_thirdpartycategories_id' => $record->fields['id']
         ]
      ];
   }
 ...
 public static function countForItem($item) {
      $dbu = new DbUtils();
      $count = 0;
         $count += $dbu->countElementsInTable('glpi_plugin_dlteams_records_externals', ['plugin_dlteams_thirdpartycategories_id' => $item->getID()]);
      return $count;
   }
