...
class PluginDlteamsAuditCategory_Item extends CommonDropdown
{
    static public $itemtype_2 = 'PluginDlteamsAudit';
    ...
    public function __construct()
    {
        ...
        static::$title = __("Elements audités", 'dlteams');
        static::$sub_title = __("Choisissez les éléments et ajouter un commentaire si besoin ", 'dlteams');
...
//            'ORDER' => self::getTable() . '.id DESC',
     ...
     *
     */
    public static function showItems(PluginDlteamsAudit $object_item)
    {
        global $DB;
...
//        Enlève le choix de L'objet LegalBasi dans la dropdown qui affiche la liste des objets
        $key = array_search("PluginDlteamsAudit", $types);
        unset($types[$key]);
        $rand = mt_rand();
        ...
        if ($canedit) {
            ...
            $types = PluginDlteamsItemType::getTypes();
            $key = array_search(PluginDlteamsAudit::class, $types);
            unset($types[$key]);
