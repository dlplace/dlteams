...
class PluginDlteamsKnowbaseItem_Item extends CommonDropdown
{
    static public $itemtype_2 = 'PluginDlteamsKnowbaseItem';
    ...
    {
        static::$itemtype_1 = str_replace("_Item", "", __CLASS__); // $itemtype_1 ---> PluginDlteamsAudit
        static::$items_id_1 = strtolower(str_replace("PluginDlteams", "", str_replace("_Item", "", __CLASS__))) . "s_id";
        static::$title = __("Ressources pour ", 'dlteams');
        static::$sub_title = __("Quels sont les textes de loi, questionnaires et liens de cet audit", 'dlteams');
...
//            ],
    ...
    {
        return __("Ressources", 'dlteams');
    }

    static function getTypeNameForClass($nb = 0)
    {
        return __("Eléments audités", 'dlteams');
    }

    public static function getTable($classname = null)
    {
        return KnowbaseItem_Item::getTable();
    }
    ...
    {
        switch ($item->getType()) {
            case 'PluginDlteamsKnowbaseItem':
                if (!$withtemplate) {
...
//        die();
     ...
     *
     */
    public static function showItems(PluginDlteamsKnowbaseItem $object_item)
    {
        global $DB;
...
//        Enlève le choix de L'objet LegalBasi dans la dropdown qui affiche la liste des objets
        $key = array_search("PluginDlteamsKnowbaseItem", $types);
        unset($types[$key]);
        $rand = mt_rand();
        ...
        if ($canedit) {
            ...
            $types = PluginDlteamsItemType::getTypes();
            $key = array_search(PluginDlteamsKnowbaseItem::class, $types);
            unset($types[$key]);
...
//        var_dump(self::getTable());
    ...
    static function showForItem(CommonDBTM $item, $withtemplate = 0)
    {
        ...
        if ($canedit) {
            ...
            echo "<input type='hidden' name='itemtype' value='" . KnowbaseItem::class . "' />";
...
//                        Computer_Item::class;
                ...
                foreach (static::$table_match_str as $key => $column) {
                    ...
                    if ($key === 0)
                        $name = "<a target='_blank' href=\"" . KnowbaseItem::getFormURLWithID($data[static::$items_id_1]) . "\">" . $name . "</a>";
...
//        purge relations
    ...
    public static function getRequest(CommonDBTM $item)
    {
        ...
        $itemtype2_item = static::$itemtype_2 . "_Item";
        $table_item_name = $itemtype2_item::getTable();
