...
class PluginDlteamsDatabase extends CommonDBTM implements
   PluginDlteamsExportableInterface{
   use PluginDlteamsExportable;

   static $rightname = 'plugin_dlteams_database';
   public $dohistory = true;
   protected $usenotepad = true;

   static function getTypeName($nb = 0) {
      return _n("Database", "Databases", $nb, 'dlteams');
   }
     ...
     function showForm($id, $options = [])
   {
	...
	echo "<table width='100%'>";
		echo "<tr>";
			...
			echo "<td>";
			PluginDlteamsDatabaseCategory::dropdown([
				'addicon'  => PluginDlteamsDatabaseCategory::canCreate(),
				'name' => 'plugin_dlteams_databasecategories_id',
				'width' => '200px',
				'value' => $this->fields['plugin_dlteams_databasecategories_id']
			]);
			echo "</td>";
	...
	echo "<table width='100%'>";
		echo "<tr>";
			...
			echo "<td>";
			PluginDlteamsDatabaseHosting::dropdown([
            'addicon'  => PluginDlteamsDatabaseHosting::canCreate(),
            'name' => 'plugin_dlteams_databasehostings_id',
            'width' => '200px',
			'value' => $this->fields['plugin_dlteams_databasehostings_id']
			]);
	  echo "</td>";

			echo "<td style='text-align:left' width='15%'>". __("Storage management", 'dlteams') . "</td>";
			echo "<td>";
			PluginDlteamsDatabaseManagement::dropdown([
            'addicon'  => PluginDlteamsDatabaseManagement::canCreate(),
            'name' => 'plugin_dlteams_databasemanagements_id',
            'width' => '200px',
			'value' => $this->fields['plugin_dlteams_databasemanagements_id']
			]);
			echo "</td>";
			echo "<td width='20%' style='text-align:right'>". " " . "</td>";
		echo "</tr>";
	...
	echo "</table>";
			...
			echo "<td>";
			PluginDlteamsDatabaseType::dropdown([
              'addicon'  => PluginDlteamsDatabaseType::canCreate(),
              'name' => 'plugin_dlteams_databasetypes_id',
              'width' => '200px',
			  'value' => $this->fields['plugin_dlteams_databasetypes_id']
			]);
			echo "</td>";*/


		 /*if(!is_numeric($this->fields['plugin_dlteams_databasetypes_id'])){
		 PluginDlteamsUtils::select2Dropdown([
            'itemtype' => "PluginDlteamsDatabaseType",
            'addicon'  => PluginDlteamsDatabaseType::canCreate(),
            'name' => 'plugin_dlteams_databasetypes_id[]',
            'entity' => $this->fields['entities_id'],
            'display_emptychoice' => false,
            'width' => '85%',
			'values' => json_decode($this->fields['plugin_dlteams_databasetypes_id']),
            'specific_tags' => [
               'multiple' => true
            ],
         ]);
...
/*****************************************Section access path (appliance) - BEGIN**/
      ...
      $iterator = $DB->request([
         'SELECT' => [
            ...
            'glpi_appliances.entities_id as entities_id',
         ...
         'WHERE' => [
			...
			'glpi_plugin_dlteams_allitems.itemtype2' => $itemtype2,
			'glpi_plugin_dlteams_allitems.itemtype1' => $this->getType()
         ],
        'ORDER' => ['name ASC'],
      ], "", true);
     ...
     // while ($data = $iterator->next()) {
	  ...
	  if ($canedit) {
         ...
         echo "<input type='hidden' name='itemtype' value='".Appliance::getType()."' />";
    ...
    echo "<tr class='tab_bg_2'><th style='text-align:center!important'>" . __("Chemins d'accès à l'entrepôt", 'dlteams') . "</th></tr>";
    echo "</table>";
	...
	echo "<table class='tab_cadre_fixe'>";
		...
		echo "<tr>";
			echo "<td style='text-align:right'>". __("Indiquer le chemin (url, chemin de dossier, lieu physique...)", 'dlteams') . " " . "</td>";
			$comment = Html::cleanInputText($this->fields['comment']);
...
/****************************************Section access path - END **/
   ...
   function rawSearchOptions() {
	  ...
	  $tab[] = [
         'id'                 => '7',
         'table'              => 'glpi_plugin_dlteams_databasecategories',
         'field'              => 'name',
	  ...
	  $tab[] = [
         'id'                 => '8',
         'table'              => 'glpi_plugin_dlteams_databasehostings',
         'field'              => 'name',
	  ...
	  $tab[] = [
         'id'                 => '9',
         'table'              => 'glpi_plugin_dlteams_databasemanagements',
         'field'              => 'name',
   ...
   public function defineTabs($options = [])
   {
	  ...
	  ->addStandardTab('PluginDlteamsDatabase_Item', $ong, $options)
      ->addStandardTab('PluginDlteamsObject_document', $ong, $options)
	  ->addStandardTab('ManualLink', $ong, $options)
	  ->addStandardTab('PluginDlteamsControllerInfo', $ong, $options)
      ->addStandardTab('Group', $ong, $options)
