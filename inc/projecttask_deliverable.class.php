...
class PluginDlteamsProjectTask_Deliverable extends CommonDBRelation
{
   // From CommonDBRelation
    public static $itemtype_1   = 'ProjectTask';
    public static $items_id_1   = 'projecttasks_id';

    public static $itemtype_2   = 'PluginDlteamsDeliverable';
    public static $items_id_2   = 'deliverables_id';
    ...
    {
        return _n('Link Deliverable/Project task', 'Links Deliverable/Project task', $nb);
    }
        ...
        if (static::canView()) {
            ...
            switch ($item->getType()) {
                case 'ProjectTask':
                    ...
                    return self::createTabEntry(PluginDlteamsDeliverable::getTypeName(Session::getPluralNumber()), $nb);
    ...
    /**
     * Get total duration of deliverables linked to a project task
     *
     * @param $projecttasks_id    integer    $projecttasks_id ID of the project task
     *
     * @return integer total actiontime
     **/
    public static function getDeliverablesTotalActionTime($projecttasks_id)
    {
        /** @var \DBmysql $DB */
        global $DB;

        $iterator = $DB->request([
            'SELECT'       => new QueryExpression('SUM(glpi_deliverables.actiontime) AS duration'),
            'FROM'         => self::getTable(),
            'INNER JOIN'   => [
                'glpi_deliverables' => [
                    'FKEY'   => [
                        self::getTable()  => 'deliverables_id',
                        'glpi_deliverables'    => 'id'
                    ]
                ]
            ],
    ...
    /**
     * Show deliverables for a projecttask
     *
    ...
    public static function showForProjectTask(ProjectTask $projecttask)
    {
        ...
        if ($canedit) {
            $condition = [
                'NOT' => [
                    'glpi_deliverables.status'    => array_merge(
                        PluginDlteamsDeliverable::getSolvedStatusArray(),
                        PluginDlteamsDeliverable::getClosedStatusArray()
                    )
                ]
            ];
        ...
        echo "<tr><th colspan='12'>" . PluginDlteamsDeliverable::getTypeName($numrows) . "</th>";
        echo "</tr>";
        if ($numrows) {
            PluginDlteamsDeliverable::commonListHeader(Search::HTML_OUTPUT, 'mass' . __CLASS__ . $rand);
            ...
            foreach ($deliverables as $data) {
                Session::addToNavigateListItems('Deliverable', $data["id"]);
                PluginDlteamsDeliverable::showShort(
                    $data['id'],
                    [
                        'row_num'                => $i,
    ...
    /**
     * Show projecttasks for a deliverable
     *
     * @param $deliverable Deliverable object
     **/
    public static function showForDeliverable(Deliverable $deliverable)
    {
        /**
         * @var array $CFG_GLPI
         * @var \DBmysql $DB
        ...
        $ID = $deliverable->getField('id');
        if (!$deliverable->can($ID, READ)) {
            return false;
        }

        $canedit = $deliverable->canEdit($ID);
        $rand    = mt_rand();

        $iterator = self::getListForItem($deliverable);
        $numrows = count($iterator);
        ...
        if (
            $canedit
            && !in_array($deliverable->fields['status'], array_merge(
                $deliverable->getClosedStatusArray(),
                $deliverable->getSolvedStatusArray()
            ))
        ) {
            $rand = mt_rand();
            ...
            $p = ['projects_id'     => '__VALUE__',
                'entity_restrict' => $deliverable->getEntityID(),
                'used'            => $used,
                'rand'            => $rand,
                'myname'          => "projects"
            ];
            ...
            $dd_params = [
                'used'        => $used,
                'entity'      => $deliverable->getEntityID(),
                'entity_sons' => $deliverable->isRecursive(),
                'displaywith' => ['id']
            ];
            ...
            echo TemplateRenderer::getInstance()->render('components/form/link_existing_or_new.html.twig', [
                'rand' => $rand,
                'link_itemtype' => __CLASS__,
                'source_itemtype' => PluginDlteamsDeliverable::class,
                'source_items_id' => $ID,
                ...
                'ajax_dropdown' => [
                    ...
                    'url' => $CFG_GLPI["root_doc"] . "/ajax/dropdownProjectTaskDeliverable.php",
                    'params' => $p
                ],
                'create_link' => false
            ]);
        ...
        if ($numrows) {
            ...
            $iterator = $DB->request([
                ...
                'LEFT JOIN' => [
                    ...
                    //'glpi_projecttasks_deliverables'   => [
                    //    'ON' => [
                    //        'glpi_projecttasks_deliverables'   => 'projecttasks_id',
                ...
                'WHERE'     => [
                    'glpi_projecttasks_deliverables.deliverables_id' => $ID
                ],
                'ORDERBY'   => [
                    "$sort $order"
                ]
