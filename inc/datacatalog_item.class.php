...
class PluginDlteamsDataCatalog_Item extends CommonDBTM
{
    ...
    public function __construct()
    {
        ...
        static::$title = __("Catalogues dont cette clé permet l'accès", 'dlteams'); // pour l'objet accountkey
        static::$sub_title = __("Choisir un catalogue", 'dlteams');
    ...
    {
        return _n('Data catalog', 'Data catalogs', 2, 'dlteams');
    }
        ...
        switch ($item->getType()) {
            ...
            case PluginDlteamsAccountKey::class:
                if (!$withtemplate) {
                    if (Session::haveRight($item::$rightname, READ)) {
                        if ($_SESSION['glpishow_count_on_tabs']) {
                            return static::createTabEntry(__("Accès Catalogues", "dlregsiter"), static::countForItem($item));
                        }
...
//
            ...
            $used = [];
            if ($item->getType() == PluginDlteamsAccountKey::class && $item->fields["plugin_dlteams_datacatalogs_id"]) {
//                enlever les catalogues de données qui ne sont pas des annuaires
                $catalog_query = [
                ...
                $catalog_iterator = $DB->request($catalog_query);
//                foreach ($catalog_iterator as $catalog)
//                    $used[$catalog["id"]] = $catalog["id"];
            }
            ...
            if ($item->getType() == PluginDlteamsAccountKey::class) {
                echo "<tr class='tab_bg_1' style='display: none;' id='field-permissions'>";
...
//
        ...
        if ($item->getType() == PluginDlteamsAccountKey::class) {
            $script .= "document.getElementById('field-permissions').style.display = 'table-row';";
        }
...
//                                    comm_field.val(comm_field.val().replace(/^\s+/, ''));
        ...
        if ($item->getType() == PluginDlteamsAccountKey::class) {
            $script .= "document.getElementById('field-permissions').style.display = 'none';";
        }
        $script .= "}});});</script>";
...
//        }
        ...
        $header_end .= "<th>" . __("Type de catalogue") . "</th>";
        $header_end .= "<th>" . __("Diffusion de l'information") . "</th>";
...
//                    $name = "--";
//
//                if ($column["column_name"] == "name" && $item->getType())
...
//
            ...
            $datacarriercategoryname = isset($data["datacarriercategory_name"])?$data["datacarriercategory_name"]:"--";
            echo "<td class='left" . (isset($data['is_deleted']) && $data['is_deleted'] ? " tab_bg_2_2'" : "'");
            echo ">" . $datacarriercategoryname . "</td>";


            $classificationname = isset($data["catalogclassification_name"])?$data["catalogclassification_name"]:"--";
            echo "<td class='left" . (isset($data['is_deleted']) && $data['is_deleted'] ? " tab_bg_2_2'" : "'");
            echo ">" . $classificationname??"" . "</td>";
...
//        $relation_item->deleteByCriteria($criteria);
    ...
    public static function getRequest(CommonDBTM $item)
    {
        ...
        if ($item->getType() == PluginDlteamsPolicieForm::class) {
            $query["SELECT"][] = $table_name . '.completename AS completename';

            $query["LEFT JOIN"][PluginDlteamsDataCarrierCategory::getTable()] = [
                'ON' => [
                    PluginDlteamsDataCarrierCategory::getTable() => 'id',
                    $table_name => 'plugin_dlteams_datacarriercategories_id'
                ]
            ];

            $query["SELECT"][] = PluginDlteamsDataCarrierCategory::getTable() . '.name AS datacarriercategory_name';




            $query["LEFT JOIN"][PluginDlteamsCatalogClassification::getTable()] = [
                'ON' => [
                    PluginDlteamsCatalogClassification::getTable() => 'id',
                    $table_name => 'plugin_dlteams_catalogclassifications_id'
                ]
            ];

            $query["SELECT"][] = PluginDlteamsCatalogClassification::getTable() . '.name AS catalogclassification_name';
        }
