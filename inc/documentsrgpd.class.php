...
class PluginDlteamsDocumentsRGPD extends CommonDBChild
{

    public function __construct()
    {
        static::forceTable('glpi_documents_items');
        parent::__construct();
    }
     ...
     *
     */
    public static function showItems(Document $item)
    {
        ...
        if ($iterator) {
            ...
            foreach ($items_list as $data) {
                if ($data['name']) {
                    ...
                    echo ">";
                    if ($data['filename']) {
                        echo "<a href='../../../front/document.send.php?docid=" . $data['id'] . "' target='_blank'>";
                        echo "voir";
                        echo "</a>";
                    } else {
                        echo "---";
    ...
    public static function getRequest(Document $document)
    {
        $request = "SELECT `glpi_documents_items`.`id` AS `linkid`, 
                            `glpi_documents_items`.`itemtype` AS `itemtype`, 
                    ...
                    WHERE `glpi_documents_items`.`itemtype` LIKE 'PluginDlteams%' 
                    AND `glpi_documents_items`.`documents_id` = " . $document->fields['id'] . "
                    ORDER BY `name` ASC;";
                ...
                foreach ($ids as $id) {
                    ...
                    if ($object_item->getFromDB($id)) {
                        ...
                        $DB->delete(
                            ...
                            array(
                                'itemtype' => 'Document',
                                'items_id' => $object_item->fields['documents_id'],
...
//                                var_dump($base_item->fields);
                                ...
                                $t = $itemtype_item->deleteByCriteria([
                                    ...
                                    "itemtype" => "Document",
                                    "items_id" => $base_item->fields["documents_id"],
