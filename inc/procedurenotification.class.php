...
class PluginDlteamsProcedureNotification extends CommonDBTM
{
...
//        echo "</div>";
        ...
        echo "</table>";
        echo "</div>";
        echo "<input type='hidden' name='procedure_id' value=\"" . $item->fields["id"] . "\">";
...
//            echo "</td>";
    ...
    public function showApprobationView($token, $error = false)
    {
        ...
        $di = new PluginDlteamsProcedure_Item();
        $dix = $di->find([
            "approval_token" => $token
        ]);

        $di_list = null;
        foreach ($dix as $procedure_item) {
            $di_list = $procedure_item;
        }
        if ($di_list) {
//            $link_item =
        }
    ...
    public static function getRequest(PluginDlteamsProcedure $item)
    {
        $request = [
            "SELECT" => [
                PluginDlteamsProcedure_Item::getTable() . ".*",
                PluginDlteamsProcedure_Item::getTable() . ".id as linkid",
            ],
            "FROM" => PluginDlteamsProcedure_Item::getTable(),
            "OR" => [
                [
                    "itemtype" => "Contact",
                    "procedures_id" => $item->fields["id"]
                ],
                [
                    "itemtype" => "User",
                    "procedures_id" => $item->fields["id"]
    ...
    {
        static::forceTable(PluginDlteamsProcedure_Item::getTable());
    }
...
//        purge relations
        ...
        $criteria = [
            ...
            "items_id" => $this->fields["procedures_id"],
    ...
    $(document).ready(function () {
        $(".btn-showcomment").on('click', function () {
            ...
            glpi_ajax_dialog({
                ...
                bs_focus: false,
                url: '/marketplace/dlteams/ajax/get_procedure_notification_data.php',
                params: {
                    comment_id,
                },
