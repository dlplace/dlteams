...
class PluginDlteamsPersonalDcp_onglet extends CommonDBTM {                                
 ...
 //var_dump($id);
      ...
      if ($iterator) {
		 ...
		 $header_end .= "<th class='left'>" . __("Categorie de personne", 'dlteams') . "</th>";
         ...
         foreach ($items_list as $data) {
			...
			echo "<td class='left'>" . ($data['gpdpname']) . "</td>";
   ...
   static function getRequest($record) {
      return [
         ...
         'WHERE' => [
            'glpi_plugin_dlteams_records_personalanddatacategories.plugin_dlteams_processeddatas_id' => $record->fields['id']
         ]
      ];                                    
   }
   ...
   public static function countForItem($item) {                                                                          
      $dbu = new DbUtils();
      $count = 0;
         $count += $dbu->countElementsInTable('glpi_plugin_dlteams_records_personalanddatacategories', ['plugin_dlteams_processeddatas_id' => $item->getID()]);
      return $count;
   }
