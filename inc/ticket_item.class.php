 ...
 **/
class PluginDlteamsTicket_Item extends CommonItilObject_Item
{
...
//            "comment" => $this->fields["comment"]
     ...
     * @param $ticket Ticket object
     **/
    public static function showForTicket(Ticket $object_item)
    {
        global $DB;
...
//            echo "<input type='hidden' name='" . static::$items_id_1 . "' value='$instID'>";
            echo "<input type='hidden' name='itemtype1' value='" . Ticket::class . "'>";
...
//            unset($types[$key]);
            ...
            Dropdown::showSelectItemFromItemtypes(['itemtypes' => $types,
                'entity_restrict' => (isset($object_item->fields['is_recursive']) && $object_item->fields['is_recursive'] ? getSonsOf('glpi_entities', $object_item->fields['entities_id'])
...
//              </style>";
            ...
            echo "<div style='display: flex; gap: 4px;'>";
            echo "<input for='storageperiod_form$rand' type='submit' name='link_item' value=\"" . _sx('button', 'Add') . "\" class='submit'>";
            echo "</div>";
            echo "</td><td width='20%' class='left'>";
            echo "</td></tr>";
...
//        var_dump(self::getTable());
        ...
        if (!count($items)) {
            ...
            $header .= "<th width='35%'>" . __("Element") . "</th>";
    ...
    public static function getItemsRequest(CommonDBTM $object_item)
    {
        ...
        $items = $DB->request([
            'FROM' => self::getTable(),
            'SELECT' => [
                Item_Ticket::getTable() . '.id',
                Item_Ticket::getTable() . '.id as linkid',
                Item_Ticket::getTable() . '.itemtype as itemtype',
                Item_Ticket::getTable() . '.items_id as items_id',
            ],
            'WHERE' => [
                Item_Ticket::getTable() . '.' . static::$items_id_1 => $object_item->fields['id']
            ],
            'ORDER' => [Item_Ticket::getTable().'.itemtype ASC'],
        ]);
...
//                        $nb = self::countForItem($item);
                        $nb = count(static::getItemsRequest($item));
                    }
    ...
    {
        return Item_Ticket::getTable($classname); // TODO: Change the autogenerated stub
    }

    /**
     * Show tickets for a projecttask
     *
     * @param $object Dlregister object
     **/
    public static function showForItem(CommonGLPI $object)
    {
        ...
        $canedit = $object->canEdit($ID);
        $rand = mt_rand();


        switch ($object->getType()) {
            case PluginDlteamsTicketTask::class:
                $object = new TicketTask();
                $object->getFromDB($ID);
                break;
        }
        $iterator = self::getListForItem($object);
        ...
        if ($canedit) {
            ...
            echo TemplateRenderer::getInstance()->render('components/form/link_existing_or_new.html.twig', [
                ...
                'source_itemtype' => $object->getType(),
                'source_items_id' => $ID,
                'target_itemtype' => Ticket::class,
                'dropdown_options' => [
                    'entity' => $object->getEntityID(),
                    'entity_sons' => $object->isRecursive(),
                    'used' => $used,
                    'displaywith' => ['id'],
                    'condition' => $condition
                ],
