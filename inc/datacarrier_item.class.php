...
class PluginDlteamsDataCarrier_Item extends CommonDBTM
{
    ...
    {
        static::$itemtype_1 = str_replace("_Item", "", __CLASS__); // $itemtype_1 ---> PluginDlteamsDataCarrier
        static::$items_id_1 = strtolower(str_replace("PluginDlteams", "", str_replace("_Item", "", __CLASS__))) . "s_id";
        static::$title = __("Supports de données en relation avec cet objet", 'dlteams');
        static::$sub_title = __("Choisir un support à lier à cet élément", 'dlteams');
    ...
    {
        return __("Data carrier", 'dlteams');
    }
...
//        Enlève le choix de L'objet LegalBasi dans la dropdown qui affiche la liste des objets
        ...
        if ($canedit) {
            ...
            echo "<div style='display: flex; gap: 4px;'>";
            echo "<input for='datacarrieritem_form$rand' type='submit' name='add' value=\"" . _sx('button', 'Add') . "\" class='submit'>";
            echo "</div>";
            echo "</td><td width='20%' class='left'>";
            echo "</td></tr>";
