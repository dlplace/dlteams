...
$dc = new PluginDlteamsProcedure_Content();
$dc->getFromDB($_POST["content_id"]);
$rand = mt_rand();
echo "<form name='procedure_form$rand' id='update_procedurecontent_form$rand' method='post'>";
...
echo "<script>
        $('#update_procedurecontent_form$rand').off('submit').submit(function(event) {
          event.preventDefault();
          tinyMCE.triggerSave();
          $.ajax({
                    url: '/marketplace/dlteams/ajax/update_procedure_paragraph.php',
                    type: 'POST',
                    data: $(this).serializeArray(),
                    success: function (html) {
                        $('#update_procedurecontent_form$rand').closest('div.modal').modal('hide');
